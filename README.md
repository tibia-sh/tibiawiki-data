...
within a major. The rest come from the generator, and `version` in `database_info` says which
release of it wrote them. From `3.1.0` that is a release of tibia.sh's copy, listed on its
[releases page](https://github.com/tibia-sh/tibiawiki-sql/releases). Earlier releases say `9.0.0`,
the upstream release.

A snapshot says when it was taken. `generate_time` in the `database_info` table is the moment
the wiki was read, and `version` there is the generator version that read it:
...
is made by CipSoft, and its game content is copyright CipSoft GmbH. From `3.1.0`, the index is
generated by tibia.sh's copy of [tibiawiki-sql](https://github.com/tibia-sh/tibiawiki-sql). Its
[releases page](https://github.com/tibia-sh/tibiawiki-sql/releases) lists each release. That copy
is based on [tibiawiki-sql](https://github.com/Galarzaa90/tibiawiki-sql) by Allan Galarza, the original project.
