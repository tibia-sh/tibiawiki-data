 ...
 * oldest published server that depends on ^N together with the packed index before every
 * publish, and pages every item through it. That sweep covers items only, not creatures,
 * NPCs, quests or spells, so the generator the shipped index was built with stays pinned
 * here as a literal. A rebuild with any other generator fails this test and cannot
 * publish, which turns a generator change into a decision someone makes rather than a
 * side effect of the next refresh.
 */
