 ...
 * server cannot read. The README decides how a generator upgrade is gated: the oldest
 * published server that depends on ^N must pass against the new index. Until that gate
 * exists, the generator the shipped index was built with is pinned here as a literal. A
 * rebuild with any other generator fails this test and cannot publish, which turns a
 * generator change into a decision someone makes rather than a side effect of the next
 * refresh.
 */
